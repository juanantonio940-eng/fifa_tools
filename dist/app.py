...
TODAS_LAS_OPCIONES = ["🔑 FIFA OTP", "🔑 UEFA OTP", "📋 Mundial Comprobantes", "📤 Comprobantes Anytickets", "📧 Lectura Correos", "🗄️ Control BD", "📄 Extracción Facturas"]
...
if st.session_state.edit_email_target:
    ...
    keys_to_clear = ["nuevo_usuario_email", "check_fifa", "check_uefa", "check_mundial", "check_anytickets", "check_lectura", "check_controlbd", "check_extraccion"]
...
if st.session_state.show_config:
    ...
    if not st.session_state.admin_authenticated:
        ...
        with col2:
            ...
            extraccion = st.checkbox(
                "📄 Extracción Facturas",
                value="📄 Extracción Facturas" in permisos_actuales if permisos_actuales else True,
                key="check_extraccion"
            )
        ...
        with col1:
            if st.button("💾 Guardar Usuario", type="primary", use_container_width=True):
                if nuevo_email:
                    ...
                    if extraccion:
                        opciones_seleccionadas.append("📄 Extracción Facturas")
...
elif pagina == "📄 Extracción Facturas":
    # Importar y ejecutar página Extracción Facturas PDF
    from modules import extraccion_factura_page
    extraccion_factura_page.render()
